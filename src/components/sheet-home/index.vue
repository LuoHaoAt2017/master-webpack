<template>
  <div id="group-asso">
    ...
    <div
      ...
      class="sortMenu"
    >
      <!-- {{isWorkFlow}} {{bizObjectStatus}} {{ shouldShowTabbar }} -->
      <ul
        v-show="shouldShowTabbar"
        ref="hwrapperGroup"
        class="sortMenu-ul"
      >
        <!-- <li class="cell" :link="formPageLink">
            <a href="javascript:;" >详情</a>
          </li> -->
        <li
          v-for="item in sortMenu"
          :ref="`cell-${item.schemaCode}`"
          :key="item.schemaCode"
          :link="item.link"
    ...
    >
      <div class="list-wrapper list-wrapper-hook">
        <div
          v-if="ItemDatas.length !== 0"
          :class="[displayMode === 2 ? 'timeline' : '']"
          class="list-container"
        >
          <!-- 顶部提示信息 -->
          <div
            class="top-tip refresh-hook"
            style="display:none;"
          >
          ...
          <div
            ref="pullUpShow"
            class="brash-more is-browsered t-c dp-font22"
            style="display:none;"
          >
            加载了10
          </div>
                  ...
                  >
                    <div class="item-product">
                      <div>
                        <h2>
                          <span
                            v-if="!Item.Name.IsCustom"
                            class="item-title-top"
                          >
                            {{ Item.Name }}
                          </span>
                          <span
                          ...
                          <div
                            v-if="Item.Status > -1"
                            class="item-status"
                            :class="[statuArr[Item.Status] && statuArr[Item.Status].bg]"
                          >
                            {{ statuArr[Item.Status] && statuArr[Item.Status].text }}
                          </div>
                        </h2>

                        <pre class="item-summary">{{ Item.Summary }}</pre>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <template v-else>
            <ul class="list-content list-content-hook">
              <template v-for="(group, index) in groupDates">
                <li :key="index" class="timeline-item">
          ...
          <div
            v-show="!HasMore && ItemDatas.length > 0"
            class="no-moredata"
          >
            没有更多数据了
          </div>
        ...
        <div
          v-if="ItemDatas.length === 0 && ready && !isRefreshing"
          class="empty-sheet-container"
        >
          <img :src="emptyImgUrl" />
          <div class="message">
            暂无数据
    ...
    <div
      ...
      class="sheet-detail"
    >
      <keep-alive :exclude="$store.state.excludeComp">
        <router-view />
      </keep-alive>
    </div>
    <div
...
export default {
  ...
  computed: {
    ...
    // shouldShowTabbar() {
    //   return typeof(this.bizObjectStatus)==='number'?
    //     (this.bizObjectStatus===1&&this.sortMenu.length>1) :
    //     false;
    // }
  },
  watch: {
  ...
  created () {
    this.initHref = this.$route.path;
    const params = { schemaCode: this.schemacode, bizObjectId: this.bizobjectId };
    this.formPageLink.params = params;
    this.formPageLink.query = params;
    this.sortMenu.push({
      schemaCode: 'Function',
      displayName: '详情',
      selected: true,
      count: 0,
      link: this.formPageLink,
    });
    ...
    if (this.$store.state.assoStashedTab) {
      for (const it of this.sortMenu) {
        if (it.schemaCode === this.$store.state.assoStashedTab) {
          this.toggleItem(it);
        }
      }
    } else {
    ...
    // this.$router.replace({
    //   path: `${this.initHref}/secFormPage`
    // });
    ...
    this.$root.eventHub.$on('form-updated', () => {
      ...
      if (this.isFunction) {
        ...
        this.$root.eventHub.$emit('data-updated');
      } else {
        this.doRefresh();
      }
    });
  ...
  mounted () {
    ...
    // 滑动到选中的那个tab中let scrollWrap = this.$refs.hwrapper;
    const scrollWrap = this.$refs.hwrapper;
    const itemDom = this.$refs[`cell-${this.CurAsso.schemaCode}`][0];
    if (scrollWrap && itemDom) {
      this.$refs.hwrapper.scrollLeft = itemDom.offsetLeft;
    }
  },
  deactivated () {
  ...
  methods: {
    ...
    toggleItem (items) {
      ...
      if (item.schemaCode === 'Function') {
        ...
        // window.location.href = this.initHref;
        // this.$router.replace({
        //   path: `${this.initHref}`
        // });
      ...
      this.$store.state.assoStashedTab = item.schemaCode;
    },
    // 打开关联表单
    openListItem (item) {
      // 判断是否存在表单首页
      ...
      if (asso && asso.EnableSheetHome && item.Status === 1) {
        ...
        this.$router.push({
          name: 'formPage',
          params: { schemaCode: item.SchemaCode, bizObjectId: item.ObjectId, mode: item.Status === 0 ? `${item.Status}` : '' },
