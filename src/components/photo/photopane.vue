<template>
  <div :class="wrapCls" >
        <Field
          ...
          @contentClick="onIconClick"
        >
          <div v-if="maxUpload" class="h3-omit h3-select-field-placeholder" :class="{'h3-hide': readOnly}">
            <i v-if="placeholderIcon" :class="placeholderIcon"></i>
            <span>{{curHolder}}</span>
          </div>
          <div slot="icon" v-if="icon" ref="uploadIcon" :class="{'h3-hide': readOnly}" :style="uploadIconStyle" class="h3-svg h3-photo-upload-icon" @click="onIconClick">
            <i :class="icon"></i>
          </div>
          <div slot="icon" v-if="!icon" ref="uploadIcon" style="display:inline-block;line-height:0;" @click="onIconClick" :class="{'h3-hide': readOnly}" :style="uploadIconStyle" class="h3-svg h3-photo-upload-icon">
        ...
        <div  class="h3-photo-list" :class="wrapListCls"   ref="list" >
          <PhotoFieldItem v-for="(item,index) in formatPhotoList" :key="index+999" v-if="photoList && photoList.length>0"
  ...
  computed: {
    ...
    isUploadDisabled() {
      return this.maxUpload
        && this.photoList.length >= this.maxUpload;
    },
    ...
    curHolder() {
      ...
      return `已上传${this.photoList.length}张图片`;
    },
  },
};
</script>
...
.h3-photo-list {
  .h3-icon {
    ...
    .px2rem(right, 8)!important;
    .px2rem(top, 8)!important;
    width: auto;
    height: auto;
  }
  svg {
