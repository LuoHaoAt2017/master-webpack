  ...
  methods: {
    ...
    handleDelete (e) {
      ...
      if (range) {
        ...
        const {
          atItems, members, suffix, deleteMatch, itemName,
        } = this;
    ...
    handleInput (keep) {
      ...
      const tempHTML = el.innerHTML.substr(0, this.maxlength);
      ...
      if (range) {
        ...
        if (el.innerHTML.length > this.maxlength) {
          const middlevalue = el.innerHTML.substr(0, this.maxlength);
          // 判断两次值是否相同,如果不同，并且已经是大于2000，那么采用真实的realcontent值
          if (this.realContent !== middlevalue && this.realContent) {
            el.innerHTML = this.realContent;
            el.blur();
            return;
          }
          this.realContent = middlevalue;
          el.innerHTML = this.realContent;
          el.blur();
        }
      }
