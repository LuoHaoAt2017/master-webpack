    ...
    <layout :title="DisplayName" :leftOptions="{preventGoBack: true}" @on-click-back="goBack">
      ...
      <div class="query-wrapper">
        <div class="query-list-content">
          ...
          <div ref="pullUpShow" class="brash-more is-browsered t-c dp-font22" style="display:none;">
            加载了10
          </div>
          <div v-show="value && searchWrapShowBol" class="search-result-num dp-font30">
            搜索结果：共{{ searchData.length }}条
          </div>
          <div v-show="value && searchWrapShowBol && searchData.length < 1" class="no-result-wrap">
          ...
          >
            ...
            <div class="item-container bd-bot">
              <div class="item-content" style="flex-grow:2;">
                <div v-if="CanSelect" class="item-select item-select-a" @click.stop.prevent="changeSelectState(Item)">
                  <i v-if="IsMultiple" class="dp-font34" :class="['h3yun_All', Item.Selected ? 'check-circle' : 'circle-o']"></i>
                  <i v-else class="dp-font34" :class="[Item.Selected ? 'icon-Path' : '']"></i>
                </div>
                <div class="item-product" :class="{'item-product-hasimage': ShowImage}">
                  <div v-if="ShowImage" class="item-product-left">
                    <img v-if="!Item.__IconUrl" src="../../assets/img/product_default.png" @click.stop.prevent="previewDefault()" />
                    <img
                      v-cloak
                      v-if="Item.__IconUrl"
                      :src="Item.__IconUrl"
                      @click.stop.prevent="preview(Item.IconId)"
                    />
                  </div>
                  <div class="item-product-body" :class="{'no-pic': !ShowImage}">
                    ...
                    <div class="item-summary">
                      <div v-for="(dicData,index) in Item.MobileDicData" :key="dicData.key" class="item-wrapper" v-if="index < 6">
          ...
          <div v-show="!HasMore && ItemDatas.length > 0" class="no-moredata">
            没有更多数据了
          </div>
        </div>
      </div>
