<template>
  ...
  >
    <div
      ...
      v-show="vCloak"
      ref="formpage"
      :class="{'bottomHide': buttonActions.length === 0 || isOpenQuery}"
    >
      <div class="form-child-wrapper">
        <workflow-state
          v-if="isCommentShow && $route.name !== 'secFormPage' && !isOpenQuery"
          class="form-comment"
...
import { getExternalStatus } from '@/light-app/service/external';
// import UserOrg from '@/light-app/pages/form/attribute/component/user-org.vue';
// add
...
export default class FormPage extends Mixins(FormCache) {
  // props 路由传参
  @Prop({ default: '' })
  ...
  // 是否是公开查询方式打开表单，3.0 添加
  get isOpenQuery () {
    return window.GlobalConfig.isOpenQuery;
  }

  // 是否显示头部， 3.0 公开查询在钉钉里面不显示头部，其他显示
  get showHeader () {
    if (this.isOpenQuery) {
      if (isDingtalk) {
        return false;
  ...
  mounted () {
     ...
     */
    // 注：禁用前端事件代码 本迭代不上 匆删
    // this.getFrontEvents(this.schemaCode); // 3.0 新增，
  }
  ...
  */
  initFormPageData (cacheData?: H3.Form.DataCache) {
    ...
    const { fromEdit, fromBoard, formDefaultValue } = params;
    this.fromEdit = fromEdit; // 编辑状态，打开已有数据表单时默认非编辑状态，只有点击编辑后，路由加上该参数，刷新页面
    this.fromBoard = fromBoard === 'true'; // 是否从看板试图跳转过来
    this.formDefaultValue = formDefaultValue ? JSON.parse(formDefaultValue) : {}; // 是否携带了表单默认值，看板试图新增进来会携带默认值， 2.0 不存在
  ...
  */
  getRequestParams (params, query) {
    ...
    requestParams.IsOpenQuery = GlobalConfig.isOpenQuery ? '1' : '0'; // 公开查询 3.0 独有
  ...
  async getFormData (requestParams) {
    ...
    if (res.Successful) {
      ...
      // 此处逻辑需要重新梳理
      // FormMode : { 0: Edit, 4: View, 1: Readonly, 2: Create, 3: Print }
      if (AssociationBOs &&
        AssociationBOs.length > 0 &&
        (ViewContext.FormMode === 1 || ViewContext.FormMode === 4) &&
        this.$route.name === 'formPage' &&
        this.$route.params.linkSource !== 'list' &&
        this.$route.params.linkSource !== 'boardEntry' &&
      ...
      // if (mappingDataFieldList && mappingDataFieldList.length) {
      //   mappingDataFieldList.forEach((item) => {
      //     if (item.indexOf('.') < 0) {
      ...
      //           subTableValue.T[item].Value = requestParams[item];
      //           if (subTableValue.R && subTableValue.R.length === 0 && Data.ViewContext.IsCreateMode) {
      //             subTableValue.R = [subTableValue.T];
      //           }
      //         }
      //       }
      //     }
      ...
      // 3.0分享填单部分控件不展示
      const form = this.$store.state.Form.ViewModel[this.bizObjectId];
      // this.newform = form;
      if (form.$responseContext.IsExternalForm && this.islightapp) {
        this.newform = this.filterFormData(form, form.$renderControls);
        const result = await getExternalStatus(this.schemaCode);
        if (result.success) {
          const setting = result.returnData.Setting;
          if (setting) {
            const title = setting.title || form.$responseContext.DisplayName;
            setTitle(title);
          }
        }
      }
      ...
      if (
        ...
        // @ts-ignore 这一串串下来.....疯了啊.....
        const msg = res.ErrorMessage || res.errorMsg || res.errorMessage;
        if (msg) {
          H3PluginDeveloper.IShowTip('提示', msg, () => { closeApp(); }, '3000');
        }
        this.goToList();
      }
    }
  ...
  async goBack () {
    ...
    if (window.GlobalConfig.isOpenQuery) {
      ...
      if (
        this.responseContext.FormMode != 4 &&
        this.responseContext.FormMode != 1
      ) {
        const viewModelForm = this.$store.state.Form.ViewModel[
          this.bizObjectId
        ];
        if (viewModelForm && viewModelForm.$dirtyChanged && this.isFormReady) {
          const self = this;
  ...
  async buttonClicked (actionItem) {
    ...
    // if (this.activityParticipants.length && actionItem.Action === 'Submit' && this.users.length === 0) {
    //   this.$h3.toast.show({ text: '请选择指定审批人' });
    //   return;
    // }
    // 重置表单的脏值检测
    this.$store.commit(
    ...
    const actionTypeRules = [
      ...
      {
        match () {
          return commentItem.Action === 'Submit';
        },
        async action () {
          const pageValid = await self.validateControls(commentItem.Action);
          const option = {};
  ...
  renderActionButtons () {
    ...
    console.log( 'Actions: ', this.buttonActions, actionsArr )
    console.log( 'Condtions: ', this.responseContext.IsCreateMode, this.isExternalFormBol, this.isExternalShareBol, (
      this.responseContext.IsCreateMode &&
      !this.buttonActions.length &&
      this.isExternalFormBol &&
      !this.isExternalShareBol
    ) );
