<template>
  <div class="sns-wrapper">
    ...
    <div class="snslist">
      ...
      :on-infinite="infinite">
        <div class="sns-item-wrap">
          <div v-for="(Item,index) in itemDatas" :key="index" class="sns-item" :class="index==itemDatas.length-1?'bd-bot':''" >
            <div class="item-wrapper" :class="index==itemDatas.length-1?'':''">
              ...
              <div class="item-content">
                <span class="reply-button" @click="reply(Item)">回复
                ...
                <div class="item-comment dp-font30 textarea-view-wraper" ref="Item" >
                  <div class="textarea-view-text" v-html="unescape(Item.Text)" ></div>

                  <div
                    style = "display: none"
                    class="textarea-view-operate-warpper"
                    @click="toogleViewText( $event )">
                    <div class="textarea-view-operate dp-font24">
                      展开<i class="icon aufont icon-base-down dp-font16"></i>
                    </div>
                  </div>
                </div>

                <div class="item-files">
                  <img v-for="(img, index) in Item.img" @click="previewImage(Item.img, index)" :id="img.ObjectId" :data-name="Item.Name" :key="img.ObjectId" :src="img.ThumbnailUrl">
    ...
    <h3-button class='send-comments' style="{'color': '#1890FF'}" @click='comment()'>
      <span class="h3yun_All conversation-o"></span>
      评论
    </h3-button>
    </layout>
    <reply-modal
...
import snsCommentItem from '../../components/sns/snsCommentItem.vue';
import replyModal from '../../components/sns/reply-modal.vue';
...
export default {
  ...
  methods: {
    html(value) {
      ...
      value = value.replace(/&nbsp;/g," ");
      value = value.replace(/&quot;/g,"'");
      value = value.replace(/dp-font34/g, 'dp-font30');
      return value;
    },
    reply(item) {
      this.replyItem = item;
    ...
    /**
     * description: 后端返回的地址是缩略图，当用户查看的时候，需要预览大图资源
    */
    previewImage (images, index) {
      const photos = images.map(img => {
        return {
          title: '',
          url: `${baseUrl}/Form/PcSnsPreview/?AttachmentID=${img.ObjectId}`,
        };
      });
      const options = {
        images: photos,
    ...
    setAutoContentHeight() {
      this.$nextTick(() => {
          //获取字符的长度
          //console.log( this.$refs )
          let $refsArr = this.$refs.Item;
          for( let i = 0; i < $refsArr.length; i++ ){
                //console.log( 'parent  ' + $refsArr[i].offsetHeight )
                //console.log( 'child   ' + $refsArr[i].firstChild.offsetHeight  )
                let parent = $refsArr[i].offsetHeight;
                let child  = $refsArr[i].firstChild.offsetHeight;
                if( child > parent ){
                    $refsArr[i].children[0].classList.add( 'textarea-view-text-overflow' );
                    $refsArr[i].children[1].style.display = 'block';
                }
          }
      })
    },
  ...
  let odivText  = ev.target.parentNode.previousElementSibling || ev.target.parentNode.parentNode.previousElementSibling;

        //实现逻辑 当有overfolw的时候，移除这个overflpw，max-height设置成none，让高度自适应
        if( odivText.className.indexOf( 'textarea-view-text-overflow' ) != -1 ){
            odivText.classList.remove( 'textarea-view-text-overflow' )

            odivText.parentNode.classList.add( 'input-view-wrapper-operating' );
            //ev.toElement.innerHTML = "收起<i class='icon aufont icon-base-up dp-font16'></i>"
            odivText.nextElementSibling.children[0].innerHTML = "收起<i class='icon aufont icon-base-up dp-font16'></i>"
        }else{
            odivText.classList.add( 'textarea-view-text-overflow' )

            odivText.parentNode.classList.remove( 'input-view-wrapper-operating' );
            //ev.toElement.innerHTML = "展开<i class='icon aufont icon-base-down dp-font16'></i>"
            odivText.nextElementSibling.children[0].innerHTML = "展开<i class='icon aufont icon-base-down dp-font16'></i>"
        }
  ...
  beforeRouteLeave(to, from, next) {
    console.log('snslist', to.path)
    if (to.path && to.path.indexOf('/sheet-home') > -1 || to.path === '/formPage') {
      this.$store.state.excludeComp = ['snslist'];
    } else {
