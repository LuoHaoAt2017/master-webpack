...
<script>
import H3FilterItem from './filter-item.vue';
...
export default {
  ...
  methods: {
    ...
    formatQueryParams(params) {
      ...
      //   res = this.remarkFilterItem(res);
      // }
      this.itemGroups = res;
    },
    // remarkFilterItem(res) {
    //   res.forEach((element) => {
    //     for (const idx in this.filterItems) {
    //       if (element.group === idx) {
    //         element.children.forEach((secElements) => {
    //           const secElement = secElements;
    //           secElement.Active = false;
    //           if (secElement.Code === this.filterItems[idx].Code) {
    //             secElement.Active = this.filterItems[idx].Active;
    //           }
    //         });
    //       }
    //     }
