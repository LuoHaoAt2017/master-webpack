<template>
  ...
  >
  <div class="workflow-view workflow-view-fillsheet">
    <div ref="header">
      <h3-tab class="h3-tab-border-color" ref="tab" :activeColor="'rgba(24,144,255,1)'" :barActiveColor="'rgba(24,144,255,1)'">
        <h3-tab-item selected @on-item-click="toggleStatus">待办({{totalTodo}})</h3-tab-item>
        <h3-tab-item @on-item-click="toggleStatus">已办</h3-tab-item>
      </h3-tab>


      <div class="search-bar" v-show="showSearchBar" ref="searchBar">
    ...
    <h3-modal :show.sync="showFilter" maskClosable type="popup" popup-direction="right" @on-hide="hideFilter" style="position: absolute">
      <workflow-filter :type="0" :state="curState" :filterItems="filterItems" category="approval" ref="filter" @reset="resetFilter" @ok="filter"></workflow-filter>
    </h3-modal>
    ...
    <h3-blank-page style="bottom:0;height:auto;" :style="wrapFilterStyle" :imageUrl="filterDodataImgSrc" :imgWidth="77" :imgHeight="77" tipText="暂无筛选结果" :showButton="true"
...
export default {
  ...
  data() {
    return {
      title: '待我经办',
      workItemType: WorkItemTypes.Fill,
      totalTodo: 0,
  ...
  beforeRouteLeave(to, from, next) {
    if (to.name === 'home' || to.name === 'singleApp') {
      this.$store.state.excludeComp = ['fillsheet'];
    } else {
