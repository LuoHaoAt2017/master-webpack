<template>
  <div :class="wrapClass" v-if="animating">
    <template v-if="toast">
      <div :class="`${prefixCls}-content`">
        <template v-if="text">
          <span :class="spinnerClass"  aria-hidden="true" />
          <span :class="`${prefixCls}-toast`">{{text}}</span>
        </template>
        <template v-else>
          <span  :class="spinnerClass" aria-label="Loading" />
        </template>
      </div>
    </template>
    <template v-else >
      <template v-if="text">
        <span :class="spinnerClass" aria-hidden="true" />
        <span :class="`${prefixCls}-tip`">{{text}}</span>
      </template>
      <template v-else>
        <span :class="spinnerClass" aria-label="loading" />
      </template>
    </template>
    
  </div>
</template>

<script>
import classnames from 'classnames';

export default {
  name: 'h3-activity-indicator',
  props: {
    prefixCls: {
      type: String,
      default: 'h3-activity-indicator',
    },
    animating: {
      type: Boolean,
      default: true,
    },
    size: {
      type: String,
      default: 'small',
    },
    panelColor: {
      type: String,
      default: 'rgba(34,34,34,0.6)',
    },
    toast: {
      type: Boolean,
      default: false,
    },
    text: {
      type: String,
    },
  },
  mounted() {
    this.state.show = true;
  },
  data() {
    return {
      state: {
        show: false,
      },
    };
  },
  computed: {
    wrapClass() {
      return classnames(this.prefixCls, {
        [`${this.prefixCls}-lg`]: this.size === 'large',
        [`${this.prefixCls}-sm`]: this.size === 'small',
        [`${this.prefixCls}-toast`]: !!this.toast,
      });
    },
    spinnerClass() {
      return classnames(`${this.prefixCls}-spinner`, {
        [`${this.prefixCls}-spinner-lg`]: !!this.toast || this.size === 'large',
      });
    },
  },
};
</script>

<style lang="less" scoped>
@import '../../styles/index';
@import '../../styles/themes/default';
@import '../../styles/mixins';

@activityIndicatorPrefixCls: h3-activity-indicator;

.@{activityIndicatorPrefixCls} {
  display: flex;
  align-items: center;
  z-index: 99;

  &-spinner {
    display: inline-block;
    width: 20 * @hd;
    height: 20 * @hd;
    .encoded-svg-background('loading');

    background-position: 50%;
    background-size: 100%;
    background-repeat: no-repeat;
    animation: spinner-anime 1s linear infinite;
  }

  &-tip {
    font-size: @font-size-base;
    margin-left: @h-spacing-md;
    color: @color-text-base;
    opacity: 0.4;
  }

  &.@{activityIndicatorPrefixCls}-toast {
    position: fixed;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    display: flex;
    justify-content: center;
    align-items: center;
    text-align: center;
    z-index: @toast-zindex;

    .@{activityIndicatorPrefixCls}-spinner {
      margin: 0;
    }

    .@{activityIndicatorPrefixCls}-toast {
      display: inline-block;
      position: relative;
      top: 4 * @hd;
    }
  }

  &-content {
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    padding: @v-spacing-lg @h-spacing-lg;
    border-radius: @radius-lg;
    background-clip: padding-box;
    color: @color-text-base-inverse;
    background-color: @toast-fill;
    font-size: @font-size-subhead;
    line-height: 20 * @hd;
  }

  &-spinner-lg {
    width: 32 * @hd;
    height: 32 * @hd;
  }

  @keyframes spinner-anime {
    100% {
      transform: rotate(360deg);
    }
  }
}
</style>
