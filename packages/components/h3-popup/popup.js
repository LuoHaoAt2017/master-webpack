...
import passiveSupported from '../../utils/passive-supported'
const isBrowser = typeof window === 'object'

// not a good way but works well
if (isBrowser) {
  window.__$h3Popups = window.__$h3Popups || {}
}
...
const popupDialog = function (option) {
  ...
  if (!!document.querySelectorAll('.h3-popup-mask').length <= 0) {
    this.divMask = document.createElement('a')
    this.divMask.className = 'h3-popup-mask'
  ...
  div.className += ` h3-popup-dialog h3-popup-dialog-${this.uuid}`
  if (!this.params.hideOnBlur) {
    div.className += ' h3-popup-mask-disabled'
  }
  ...
  this.container = document.querySelector('.h3-popup-dialog-' + this.uuid)
  this.mask = document.querySelector('.h3-popup-mask')
...
popupDialog.prototype.show = function () {
  if (this.params.showMask) {
    this.mask.classList.add('h3-popup-show')
    this.mask.style['zIndex'] = 500
  }
  this.container.classList.add('h3-popup-show')
  this.params.onOpen && this.params.onOpen(this)
  if (isBrowser) {
    window.__$h3Popups[this.uuid] = 1
  }
}

popupDialog.prototype.hide = function (shouldCallback = true) {
  this.container.classList.remove('h3-popup-show')
  if (!document.querySelector('.h3-popup-dialog.h3-popup-show')) {
    this.mask.classList.remove('h3-popup-show')
    setTimeout(() => {
  ...
  this.isShow = false
  if (isBrowser) {
    delete window.__$h3Popups[this.uuid]
  }
}
