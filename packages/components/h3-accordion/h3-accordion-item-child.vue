<template>
  <div class="list list-borderless list-accordion":style="wrpsStyle">
    <item class="item-icon-right itemChildTitleStatic" :class="itemChildTitleCls"  v-if="position === 'right'" @click.native="onClick()">
      <div class="list-accordion-title-wrapper">
        <span v-text="title" v-if="title"></span>
        <slot name="title"></slot>
      </div>
      <i class="icon aufont " :class="wrapIconClass"></i>
    </item>
    <item class="item-icon-left itemChildTitleStatic"  :class="itemChildTitleCls"  v-else @click.native="onClick()">
      <i class="icon aufont " :class="wrapIconClass"></i>
      <div class="list-accordion-title-wrapper">
        <span v-text="title" v-if="title"></span>
        <slot name="title"></slot>
...
export default {
  ...
  mounted() {
    ...
    channel.$on('activeIndexSubChanged', (activeIndex) => {
    ...
    onClick() {
      ...
      if (this.expanded) {
        if (!this.$parent.multi) {
          channel.$emit('activeIndexSubChanged', this.index);
        }
      }
      if (this.onClickFun) {
        this.onClickFun();
