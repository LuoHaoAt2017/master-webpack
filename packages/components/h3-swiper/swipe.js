...
class Swiper {
  constructor(options) {
    ...
    this.$container = this._options.container.querySelector('.h3-swiper');
  ...
  _setTransform(offset) {
    ...
    arrayFrom(me.$items).forEach(($item, key) => {
      ...
      let transform;
      let scale;
      if (me._options.mode !== 'normal') {
        distance *=  0.8;
        if (me._current === key) {
          scale = (1 -  Math.abs (distance/ me._width));
        } else {
          scale = (1 -  Math.abs (distance/ me._width) + 0.5);
        }
        if (distance !== 0 ) {
          // transform = `translate3d(${distance}px, 0, 0) scaleY(0.5)`;
          transform = `translate3d(${distance}px, 0, 0) scaleY(${scale})`;
          if (me._options.direction === 'vertical') {
            // transform = `translate3d(0, ${distance}px, 0) scaleX(0.5)`;
            transform = `translate3d(${distance}px, 0, 0) scaleX(${scale})`;
          }
        } else {
  ...
  _bind() {
    ...
    me.touchmoveHandler = (e) => {
      ...
      if (!me._options.loop && (me._current === me.count - 1 || me._current === 0)) {
        distance /= 3;
      }
