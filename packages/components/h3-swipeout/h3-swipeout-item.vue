<template>
  ...
  @mouseup="end"
  @touchcancel="end">
    <div class="h3-swipeout-button-box h3-swipeout-button-box-left" :style="leftButtonBoxStyle" v-show="distX >= 0">
      <slot name="left-menu"></slot>
    </div>
    <div class="h3-swipeout-button-box" :style="rightButtonBoxStyle" v-show="distX <= 0">
      <slot name="right-menu"></slot>
    </div>
    <div class="h3-swipeout-content" :style="styles" @mousedown="onContentClick" @touchstart="onContentClick" ref="content">
  ...
  methods: {
    ...
    caculateMenuWidth(direction) {
      ...
      if (this.$slots[`${direction}-menu`] &&
       this.$slots[`${direction}-menu`][0] &&
       this.$slots[`${direction}-menu`][0].children.length > 0) {
    ...
    move(ev) {
      ...
      // 判断水平和垂直方向的比值来确定是否显示菜单
      // if ((Math.abs(this.distX) / Math.abs(this.distY)) < 1.73) {
      //   ev.preventDefault();
      //   return;
      // }
      if (!this.direction) {
      ...
      // 计算允许左滑和右滑允许的最大距离
      const width = this.caculateMenuWidth(this.direction);
      if (Math.abs(this.distX) > width + 30) {
        ev.preventDefault();
        return;
      }
      ...
      if (this.valid === undefined) {
        if (this.distX > 0 && this.hasLeftMenu === false) {
          ...
          this.valid = false;
        } else if (Math.abs(this.distX) > this.sensitivity ||
      ...
      if (this.valid === true) {
        if (this.distX < 0 && this.direction === 'right') {
          const threshold =
          this.threshold <= 1 ? this.rightMenuWidth * this.threshold : this.threshold;
          ...
          const threshold =
           this.threshold <= 1 ? this.leftMenuWidth * this.threshold : this.threshold;
          if (this.distX > threshold) {
    ...
    setOffset(x, animated = false, force) {
      ...
      if ((this.direction === 'right' && tmpX > 0) || (this.direction === 'left' && tmpX < 0)) {
        tmpX = 0;
      }
      if (tmpX === 0) {
        setTimeout(() => {
      ...
      if (tmpX < 0 && Math.abs(tmpX) === this.rightMenuWidth) {
        this.distX = -this.rightMenuWidth;
      } else if (tmpX > 0 && Math.abs(tmpX) === this.leftMenuWidth) {
        this.distX = this.leftMenuWidth;
      }
      if (animated && this.target) {
        this.target.classList.add('h3-swipeout-content-animated');
        const cb = (function (self, target) {
          return function () {
            const tmp = self;
            target.classList.remove('h3-swipeout-content-animated');
            tmp.isAnimated = false;
      ...
      this.styles.transform = `translate3d(${tmpX}px, 0, 0)`;
    },
    setClose(delay = 0) {
