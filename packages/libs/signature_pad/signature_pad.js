...
function SignaturePad(canvas, options) {
  ...
  this.needClip = opts.needClip;
  this.clipV = opts.clipV;
  this.clipH = opts.clipH;
...
SignaturePad.prototype.getContentRect = function(direction) {
  // 获取轨迹数组的范围, 获取y轴方向的最大最小值
  var findMaxMin = function(source, attrName) {
    let arr = [];
    if(attrName) {
      for (let i = 0, len = source.length; i < len; i += 1) {
        arr.push(source[i][attrName]);
      }
    } else {
      arr = source;
    }
  ...
  if (this._data.length === 1 && this._data[0].length === 1) {
    return [this._data[0][0][direction], this._data[0][0][direction]];
  }
  const source = [];
  for (let i = 0, len = this._data.length; i< len; i += 1) {
    const tmp = this._data[i];
    for (let j = 0, len2 = tmp.length; j < len2; j += 1) {
      source.push(tmp[j]);
    }
  }
  var res = findMaxMin(source, direction);
  return res;
};

SignaturePad.prototype.save = function() {
  var targetData = null;
  var right =0, left=0;
  var topBottom = this. getContentRect('y');
  var top = topBottom[1] - this.clipV;
  top = top < 0? 0 : top;
  var bottom = topBottom[0] + this.clipV;
  bottom = bottom > this._canvas.height ? this._canvas.height: bottom;
  if (this.clipH > 0) {
    var leftRight = this.getContentRect('x');
    left = leftRight[1] - this.clipH;
    left = left < 0? 0 : left;
    right = leftRight[0] + this.clipH;
    right = right > this._canvas.width ? this._canvas.width: right;
    targetData = this._ctx.getImageData(left, top,(right - left), (bottom - top));
  } else {
    targetData = this._ctx.getImageData(0, top,this._canvas.width, (bottom - top));
  }
  var canvas = this.createCanvas(this.clipH > 0 ? (right - left) : this._canvas.width, (bottom - top));
...
SignaturePad.prototype.createCanvas = function(width, height) {
  var canvas = document.createElement('canvas');
  canvas.id = "tmpSignatureCanvas";
  canvas.width = width;
  canvas.height = height;
  document.body.appendChild(canvas);
  return canvas;
};
SignaturePad.prototype.removeCanvas = function() {
  var canvas =document.getElementById('tmpSignatureCanvas');
  if (canvas) {
